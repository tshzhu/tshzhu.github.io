...
def procfile(f):
  ...
  if showfooter and (showlastupdated or showsourcelink):
    ...
    if showlastupdated:
      ...
      # When date is hidden but source is shown, still render the
      # "Page generated by jemdoc+MathJax" text without the date.
      # Remove the date placeholder ("|") and adjoining punctuation/spaces.
      lastupdated_tpl = f.conf['lastupdated']
      lastupdated_no_date = re.sub(r'\s*\|[,]*', '', lastupdated_tpl)
      hb(f.outf, lastupdated_no_date, '')
    if showsourcelink:
